 ...
 *
 * Robust positioning strategy (shared with the other Qaiyo plugins):
 * - Each Qaiyo plugin calls Qcg_Brand_Menu::register_plugin_slug( $top_level_menu_slug )
 *   when it registers its menu. The slug is stored in $GLOBALS so it survives
 *   class-prefix differences across plugins (Qcg_/Qt_/Qtms_/Wpam_… all share one list).
 ...
 * detected by their stable SLUGs in $menu, and CSS is printed only once via a
 * shared $GLOBALS flag.
 *
 * @package qaiyo-clean-gallery
 */
...
if ( ! class_exists( 'Qcg_Brand_Menu' ) ) {
	...
	class Qcg_Brand_Menu {
		 ...
		 *
		 * @param string $slug e.g. "edit.php?post_type=clean_gallery"
		 */
		...
		private static function chip_data_uri() {
			$label       = (string) __( 'Qaiyo Plugins', 'qaiyo-clean-gallery' );
